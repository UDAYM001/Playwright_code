...
ADDRESS_MAP = {
    ...
    "2": {  # Radiology                               ###### mca
        "address": "915 456 BROADWAY AVE 900",
        "city": "HOUVDSTON",
        "zip": "77024",
        "state": "TX"
    },
    "3": {  # Orthopedics                          ######### cls
        "address": "905 W M456 BROADWAY AVE BLVD STE 102",
        "city": "WEDBSTER",
        "zip": "77598",
        "state": "TX"
    }
            ...
            try:
                ...
                if questions_present:
                    ...
                    if cpt_code == "93306":
                        ...
                        # === OPTIONAL: Try to click "No", but skip if not visible ===
                        no_locator = page.locator('span.control-caption', has_text=re.compile(r"^No$", re.IGNORECASE))
                        if no_locator.is_visible(timeout=3000):
                            no_locator.click()
                            print("Clicked 'No' for 93306.")
                        else:
                            print("'No' option not found for 93306. Skipping...")
                        ...
                        # === OPTIONAL: Try to click "No", but skip if not visible ===
                        no_locator = page.locator('span.control-caption', has_text=re.compile(r"^No$", re.I))
                        if no_locator.is_visible(timeout=3000):
                            no_locator.click()
                            print("Clicked 'No' for 78452.")
                        else:
                            print("'No' option not found for 78452. Skipping...")
                        ...
                        time.sleep(1)
                        page.locator('div:has(b:has-text("EKG abnormalities")) label:has(span:has-text("No"))').click()
